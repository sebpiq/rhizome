...
_.extend(Client.prototype, coreUtils.ValidateConfigMixin, {
  ...
  start: function(done) {
    ...
    this._validateConfig(function(err) {
      ...
      self._sendToServer = new moscow.createClient(self._config.serverHostname,
      ...
      self._receiveFromServer = new moscow.createServer(self._config.blobsPort, 'tcp')
  ...
  _onMessage: function (address, args, rinfo) {
    ...
    // but with filenames instead of blobs.
      ...
      async.parallel(args.map(function(arg) {
        ...
        if (!sendToApp) {
          sendToApp = self._sendToApps[appPort] = new moscow.createClient('127.0.0.1', appPort, 'udp')
