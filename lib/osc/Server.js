...
_.extend(OSCServer.prototype, coreValidation.ValidateConfigMixin, coreServer.Server.prototype, {
      ...
      async.waterfall([
        ...
        (ids, next) => {
          async.each(ids, (id, nextId) => {
            var parts = id.split(':')
              , connection = new OSCConnection(this, parts[0], parseInt(parts[1], 10))
  ...
  _onMessage: function (address, args, rinfo) {
    ...
    if (coreMessages.sysAddressRe.exec(address)) {
      ...
      if (!connection) {
        connection = new OSCConnection(this, ip, appPort)
...
var OSCConnection = function(server, ip, appPort) {
  coreServer.Connection.call(this, server)
  this.ip = ip
  this.appPort = appPort
  this.id = ip + ':' + appPort
  this.appClient = transport.createClient(ip, appPort, 'udp')
