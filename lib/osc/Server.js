...
_.extend(OSCServer.prototype, coreValidation.ValidateConfigMixin, coreServer.Server.prototype, {
  ...
  start: function(done) {
    ...
    this.validateConfig(function(err) {
      ...
      self._server = new transport.createServer(self._config.port, 'udp')
      ...
      self._blobsServer = new transport.createServer(self._config.blobsPort, 'tcp')
  ...
  _onMessage: function (address, args, rinfo) {
    ...
    if (coreMessages.sysAddressRe.exec(address)) {
      ...
      if (appPort === this._config.port)
        return console.error('Please keep port ' + appPort + ' reserved for the rhizome server')
...
var OSCConnection = function(ip, appPort) {
  ...
  this.id = ip + ':' + appPort
  this.appClient = new transport.createClient(ip, appPort, 'udp')
}
...
_.extend(OSCConnection.prototype, coreServer.Connection.prototype, {
  ...
  setBlobClient: function(blobsPort) {
    ...
    this.blobClient = new transport.createClient(this.ip, blobsPort, 'tcp')
