...
var fs = require('fs')
  ...
  , getFreeFilePath = require('../server/utils').getFreeFilePath

var receiveFromServer, sendToServer

exports.start = function(config, done) {

  // Listens messages coming from the rhizome server
  ...
  receiveFromServer.on('message', function (address, args, rinfo) {
    ...
    if (shared.sysAddressRe.exec(address)) {
      ...
      // to the Pd client.
      if (address === shared.takeBlobAddress) {
        ...
        var pdPort = args[0]
          ...
          , userId = args[3]
        async.waterfall([
          function(next) { getFreeFilePath(config.desktopClient.blobsDirName, next) },
          ...
          sendToPd.send(originalAddress, [filePath])
        })
      }

      // Open the file and send the blob to the server.
      ...
      // TODO: limit to files in `blobsDirName`
        ...
        fs.readFile(filePath, function(err, buf) {
          sendToServer.send(shared.takeBlobAddress, [originalAddress, buf])
        })
      }
    }
