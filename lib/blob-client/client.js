...
exports.start = function(config, done) {
  ...
  sendToApps = config.appPorts.map(function(appPort) {
    return new oscCore.createOSCClient('127.0.0.1', appPort, 'udp')
  })
  ...
  receiveFromServer.on('message', function (address, args, rinfo) {
    ...
    // but with filenames instead of blobs.
      ...
      async.parallel(args.map(function(arg) {
        ...
        sendToApps.forEach(function(sendToApp) { sendToApp.send(address, args) })
      })
    }
