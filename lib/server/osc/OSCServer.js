...
_.extend(OSCServer.prototype, Server.prototype, {
  ...
  start: function(config, done) {
    ...
    this._server = new oscCore.createOSCServer(config.oscPort, 'udp')
    ...
    this._blobsServer = new oscCore.createOSCServer(config.blobsPort, 'tcp')
  ...
  _onMessage: function (address, args, rinfo) {
    ...
    if (shared.sysAddressRe.exec(address)) {
      ...
      if (!connection)
        connection = this.addConnection(new OSCConnection({ appPort: appPort, ip: ip }, this))
