...
exports.start = function(config, done) {
  ...
  oscClients = config.osc.clients.map(function(client) {
    return new utils.OSCClient(client.ip, client.port)
  })

  desktopClients = config.osc.clients.map(function(client) {
    return new utils.OSCClient(client.ip, config.desktopClient.port)
  })
      ...
      socket.on('message', function(msg, flags) {
        if (!flags.binary) {
          ...
          desktopClients.forEach(function(client, i) {
            var args = [config.osc.clients[i].port, originalAddress, msg, socket.rhizome.userId]
