...
exports.start = function(config, done) {
  ...
  desktopClients = config.osc.clients.map(function(client) {
    return new osc.Client(client.ip, config.desktopClient.port)
  })
  ...
  desktopClients.send = function(address, args) {            
    this.forEach(function(client) {
      client.send.apply(client, [address].concat(args))
    })
  }
      ...
      socket.on('message', function(msg, flags) {
        if (!flags.binary) {
          ...
          desktopClients.forEach(function(client, i) {
            client.send(shared.takeBlobAddress, config.osc.clients[i].port, originalAddress, msg, socket.rhizome.userId)
          })
