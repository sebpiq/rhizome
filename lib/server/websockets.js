...
var wsServer
  , idManager, nsTree
  , oscServer, oscClients, desktopClients
...
exports.start = function(config, done) {
  ...
  oscClients = config.clients.map(function(client) {
    return new utils.OSCClient(client.hostname, client.oscPort)
  })

  desktopClients = config.clients.map(function(client) {
    return new utils.OSCClient(client.hostname, client.desktopClientPort)
  })
    ...
    if (socket.rhizome.userId !== null) {
      ...
      socket.on('message', function(msg, flags) {
        if (!flags.binary) {
          ...
          desktopClients.forEach(function(client, i) {
            var args = [originalAddress, msg, socket.rhizome.userId]
            client.send(shared.fromWebBlobAddress, args)
          })
