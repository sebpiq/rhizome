...
var Connection = module.exports = function(server) {
  this.server = server
...
_.extend(Connection.prototype, EventEmitter.prototype, {
  ...
  onOpened: function() {
    this.emit('opened')
    this.server.debug('closed - now ' + this.server.connections.length + ' sockets')
    connections.send(shared.connectionOpenAddress, [this.userId || 0])
    this.server._addConnection(this)
  },
  ...
  onClosed: function() {
    ...
    this.server.debug('closed - now ' + this.server.connections.length + ' sockets')
    this.server._removeConnection(this)
  },
