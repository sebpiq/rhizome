...
_.extend(Connection.prototype, EventEmitter.prototype, {
  ...
  open: function() {
    ...
    if (this.id === null && this.autoId) {
      asyncOps.push(async.whilst.bind(async, 
        function() { return !self.id },
        function(nextPick) {
          var id = coreUtils.getRandomString()
          // Ensures unicity of the connection id
          connections.manager.connectionExists(self.namespace, id, function(err, exists) {
            if (err) return nextPick(err)
            if (exists) return nextPick()
            self.id = id
            nextPick()
          })
        }
      ...
      if (address === coreMessages.subscribeAddress) {
        ...
        if (err) self.send(coreMessages.errorAddress, [err])
          ...
          if (countBefore < self._subscriptions.length)
            connections.manager.save(self, function(err) { if (err) self.emit('error', err) })
        }

      // Resends last messages received at the given address.
      } else if (address === coreMessages.resendAddress) {
        var fromAddress = args[0]
        self.send(fromAddress, connections.manager.resend(fromAddress))
      }
    }
