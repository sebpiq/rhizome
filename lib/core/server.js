...
_.extend(Connection.prototype, EventEmitter.prototype, {
  ...
  open: function() {
    var self = this
    connections.manager.open(this, function(err) {
      if (err) return self.emit('error', err)
      self.emit('open')
      connections.manager.send(coreMessages.connectionOpenAddress + '/' + self.namespace, [self.id])
    })
  },
  ...
  close: function() {
    ...
    connections.manager.close(this, function(err) {
      ...
      connections.manager.send(coreMessages.connectionCloseAddress + '/' + self.namespace, [self.id])
    })
  },
