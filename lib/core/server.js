...
_.extend(Connection.prototype, EventEmitter.prototype, {
  ...
  close: function() {
    var self = this
    connections.close(this, function(err) {
      if (err) return self.emit('error', err)
      self.emit('close')
      self.removeAllListeners()
      connections.send(coreMessages.connectionCloseAddress, [self.id || 9999])
    })
  },
    ...
    if (address === coreMessages.subscribeAddress) {
      ...
      if (err) this.send(coreMessages.errorAddress, [err])
        ...
        if (countBefore < this._subscriptions.length)
          connections.save(this, function(err) { if (err) self.emit('error', err) })
      }
  ...
  deserialize: function(data) {
    ...
    this.once('open', function() {
      self._subscriptions.forEach(function(address) {
        var err = connections.subscribe(self, address)
...
_.extend(Server.prototype, EventEmitter.prototype, {
  ...
  open: function(newConnection) {
    ...
    connections.open(newConnection, function(err) {
      if (err) return self.emit('error', err)
      self.emit('connection', newConnection)
      newConnection.emit('open')
      connections.send(coreMessages.connectionOpenAddress, [newConnection.id || 9999])
    })
  },
