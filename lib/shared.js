...
var _ = require('underscore')
  , Blob = typeof window === 'undefined' ? Buffer : window.Blob
...
exports.sendBlobAddress = '/sys/blob/send'
...
var BlobTransaction = exports.BlobTransaction = function(socket, commandName, eventEmitter) {
  this.eventEmitter = eventEmitter
  this.socket = socket
  this.commandName = commandName
  // `_locked` helps to make sure that there isn't several blob transactions happening in parallel.
  this._locked = false
  this._queue = []
}
...
_.extend(BlobTransaction.prototype, {
  ...
  send: function(address, args) {
    this._queue.push({address: address, args: args})
  ...
  _nextTransaction: function() {
    if ((!this._locked) && this._queue.length > 0) {
      this._locked = true
      var self = this
        , msg = this._queue.shift(), args = msg.args
      ...
      self.eventEmitter.once('command:' + this.commandName, function(msg) { _sendCurrentBlobs() })
      var _sendCurrentBlobs = function() {
        self.socket.send(blobs.shift())
        self.eventEmitter.once('command:' + self.commandName, function(msg) {
          if (blobs.length) _sendCurrentBlobs() 
          else {
            // The whole message and blobs have been sent, so it is safe to unlock.
            self._locked = false
            if (msg.status === 0) self._nextTransaction()
