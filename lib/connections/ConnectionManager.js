...
_.extend(ConnectionManager.prototype, coreValidation.ValidateConfigMixin, {
  ...
  open: function(connection, done) {
    ...
    this._config.store.connectionExists(connection.namespace, connection.id, function(err, exists) {
      if (err) return done(err)
      if (exists) self._config.store.connectionRestore(connection, _doFinally)
  ...
  // Calls `done(err, exists)` with exists `true` if the connection exists,
  // `false` otherwise.
  connectionExists: function(namespace, id, done) {
    this._config.store.connectionExists(namespace, id, done)
  },
