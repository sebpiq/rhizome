...
var ConnectionManager = module.exports = function(config) {
  ...
  // Maintain a counter of ids per namespaces for assigning unique ids to connections
  this._idCounters = {}
...
_.extend(ConnectionManager.prototype, coreValidation.ValidateConfigMixin, {
  ...
  start: function(done) {
    var self = this
    async.series([
      this._validateConfig.bind(this),
      ...
      if (restoredManagerState !== null) {
        ...
        self._idCounters = restoredManagerState.idCounters
      }
      ...
      self._storeWriteInt = setInterval(function() {
        ...
        // Manager state
        var managerState = { nsTree: self._nsTree.toJSON(), idCounters: self._idCounters }
  ...
  // Retruns a free id for a connection in `namespace`. An id counter
  // is persisted so that even if the server crashes and restarts, it won't
  // assign the same id twice.
  getFreeId: function(namespace) {
    if (!this._idCounters.hasOwnProperty(this.namespace))
      this._idCounters[this.namespace] = 0
    return (this._idCounters[this.namespace]++).toString()
  },
