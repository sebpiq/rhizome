...
_.extend(NEDBStore.prototype, {
  ...
  eventInsert: function(events, done) {
    events = events.map((event) => _.pick(event, ['timestamp', 'id', 'namespace', 'eventType']))
    this._eventsCollection.insert(events, (err) => done(err))
  },

  eventList: function(done) {
    this._eventsCollection.find({}).sort({timestamp: 1}).exec((err, events) => {
      if (err) return done(err)
      done(null, events.map((event) => {
