...
_.extend(BaseStore.prototype, {
  ...
  // Saves the connection in and calls `done(err)`.
  connectionSave: function(connection, done) {},
  ...
  connectionRestore: function(connection, done) {},

  // Save an event in the storage. `done(err)` is optional.
  // `event` has the fields : timestamp, eventType, namespace and id
...
_.extend(BaseStore.prototype, {
  ...
  connectionSave: function(connection, done) {done()},
  connectionRestore: function(connection, done) {done()},
...
_.extend(NEDBStore.prototype, {
  ...
  connectionSave: function(connection, done) {
    var self = this
    this.connectionExists(connection.namespace, connection.id, function(err, exists) {
      if (err) return done(err)

      var _doFinally = function(err) { done(err) }
        , connectionData = connection.serialize()
        , doc = {_id: self._getId(connection.namespace, connection.id), data: connectionData}

      if (!exists)
        self._connectionsCollection.insert(doc, _doFinally)
      else {
        var query = {_id: self._getId(connection.namespace, connection.id)}
  ...
  connectionRestore: function(connection, done) {
    var query = {_id: this._getId(connection.namespace, connection.id)}
    this._connectionsCollection.findOne(query, function(err, doc) {
      if (err) return done(err)
      if (doc) {
        connection.deserialize(doc.data)
        done()
      } else done(new Error('connection does not exist ' + connection.namespace + ':' + connection.id))
    })
  },

  eventInsert: function(event, done) {
    event =_.pick(event, ['timestamp', 'id', 'namespace', 'eventType'])
