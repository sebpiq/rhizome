    ...
    createSocket(function(err) {
      if (err) {
        ...
        nsTree.get('/').forEach(function(ns) {
          doSubscribe(ns.address, function(err) { if (err) throw err }) // TODO : better err handling
        })
        armReconnect()
      }
    })
...
var _assertValidArgs = function(args) {
  var argsErr = shared.validateArgs(args)
  if (argsErr !== null) throw new Error(argsErr)
}
