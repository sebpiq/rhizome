...
exports.config = {
  ...
  queueIfFull: function(value) {
    if (!socket) {
      if (_.isBoolean(value)) config.queueIfFull = value
  ...
  port: function(port) {
    if (!socket) {
      if (_.isNumber(port)) config.port = port
  ...
  hostname: function(hostname) {
    if (!socket) {
      if (_.isString(hostname)) config.hostname = hostname
...
var connect = function() {
  socket = new WebSocket('ws://' + config.hostname + ':' + config.port + '/'
    + '?' + 'queueIfFull=' + JSON.stringify(config.queueIfFull))
