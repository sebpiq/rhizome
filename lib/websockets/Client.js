...
var Client = module.exports = function(config) {
  ...
  this.userId = null
  _.defaults(config, { reconnect: 1000, queueIfFull: true })
...
_.extend(Client.prototype, EventEmitter.prototype, {
  ...
  start: function(done) {
    ...
    this._rhizomeEvents = new EventEmitter
    this._connect(function(err) {
      if (!err) self.emit('connected')
      if (done) done(err)
    })

    this._rhizomeEvents.once('connection:success', function() {
      self._rhizomeEvents.removeAllListeners('connection:failure')

    })

    this._rhizomeEvents.once('connection:failure', function(err) {
  ...
  stop: function(done) {
    ...
    if (this._socket) {
      if (this._socket.readyState === this._socket.OPEN) {
        ...
        this._socket.removeListener('close', this._onConnectionLost)
        this._socket.close()
  ...
  _validateConfig: function(config) {
    if (!_.isNumber(config.reconnect))
      throw new Error('`reconnect` should be a number')

    if (!_.isBoolean(config.queueIfFull))
      throw new Error('`queueIfFull` should be boolean')
  ...
  _connect: function(done) {
    ...
    if (isBrowser) _emitterSocket(this._socket)
    this._blobTransaction = new BlobTransaction(this._socket, 'blobFromWeb', 'blobFromServer', this._rhizomeEvents)

    this._socket.on('message', _.bind(this._onSocketMessage, this))

    this._socket.once('open', function(event) {
      self._socket.removeAllListeners('error')
      self._socket.on('error', _.bind(self._onSocketError, self))
      self._rhizomeEvents.once('command:connect', _.bind(self._doRhizomeConnection, self, done))
    })

    this._socket.once('error', function(event) {
      self._socket.removeAllListeners('open')
  ...
  _doRhizomeConnection: function(done, msg) {
    ...
    if (msg.status === 0) {
      ...
      this._socket.once('close', this._onConnectionLost)
      ...
      // Otherwise, we don't queue, close the connection and return an error as the connection failed.
        ...
        if (done)
          this._socket.once('close', done.bind(this, new Error(msg.error)))
        this._socket.close()
      }
    }
  },
...
var _emitterSocket = function(socket) {
  _.extend(socket, new EventEmitter)
  socket.onerror = function(event) { socket.emit('error') }
  socket.onmessage = function(event) { socket.emit('message', event.data) }
  socket.onclose = function(event) { socket.emit('close') }
  socket.onopen = function(event) { socket.emit('open') }
}